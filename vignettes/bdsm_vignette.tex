...
E(\beta|\text{data})=\sum_{j=1}^{2^K}\widehat{\beta}_{j} \cdot P(M_{j}, \pi_{\beta} = 1 |\text{data})
\end{equation}
...
because the probability $P(M_{j}, \pi_{\beta} = 1 |\text{data})$ wil be zero anyway.

The posterior variance of the parameter $\beta$ is equal to:

\begin{equation}
\begin{split}
V(\beta|\text{data}) =\ &\sum_{j=1}^{2^K}V(\beta_{j}|\text{data},M_{j}) \cdot P(M_{j}, \pi_{\beta} = 1 |\text{data})\\[1ex]
&+\sum_{j=1}^{2^K}\Bigl[\widehat{\beta}_{j}-E(\beta|\text{data})\Bigr]^{2} \cdot P(M_{j}, \pi_{\beta} = 1 |\text{data})
\end{split}
\end{equation}
...
\begin{equation}
E(\beta | \pi_{\beta}=1,\text{data})=\frac{E(\beta|\text{data})}{P(\pi_{\beta} = 1|\text{data})}.
\end{equation}
Similarly, the conditional variance is:

\begin{equation}
    V(\beta|\pi_{\beta}=1,\text{data})=\frac{V(\beta|\text{data})+E(\beta|\text{data})^2}{P(\pi_{\beta}=1|\text{data})}-E(\beta|\pi_{\beta}=1,\text{data})^2
\end{equation}
