...
the Bayesian information criterion, and the standard errors for each model.
...
Then the numerical optimisation used to find MLEs can be computed in parallel,
